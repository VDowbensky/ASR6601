...
int8_t radio_get_rx_packet(void)
{
	uint8_t rxpointer;
	uint8_t dummy;
	SX126X_GetRxBufferStatus(&dummy,&rx_paylen,&rxpointer);
...
void prepareTxPacket(void)
{
  ...
  //fill SX126x TX buffer
	SX126X_SetLoRaPacketParams(radioConfig.LoRaPreLen, radioConfig.LoRaImplHeader,txlen,radioConfig.LoRaCrcOn,radioConfig.LoRaInvertIQ);
  SX126X_writeBuffer(0,radio_txbuffer,txlen);
}
