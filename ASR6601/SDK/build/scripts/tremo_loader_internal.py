...
class TremoLoader(object):
    ...
    def write_option0(self, option0l, option0h):
        self.requeset(self.CMD_WROPT0, struct.pack('<II', option0l, option0h))
        ret, _ = self.wait_response()
        if ret != 0:
            raise CmdException("Write_option0 error")

    def read_option0(self):
        self.requeset(self.CMD_RDOPT0)
        ret, rsp = self.wait_response()
        if ret != 0:
            raise CmdException("Read_option0 error")
        return struct.unpack('<II', rsp)

    def write_option1(self, option1l, option1h):
        self.requeset(self.CMD_WROPT1, struct.pack('<II', option1l, option1h))
        ret, _ = self.wait_response()
        if ret != 0:
            raise CmdException("Write_option1 error")

    def read_option1(self):
        self.requeset(self.CMD_RDOPT1)
        ret, rsp = self.wait_response()
        if ret != 0:
            raise CmdException("Read_option1 error")
        return struct.unpack('<II', rsp)
...
def tremo_write_option0(args):
    tremo = TremoLoader(args.port)
    tremo.connect()
    tremo.set_baudrate(args.baud)
    tremo.write_option0(args.option0l, args.option0h)


def tremo_read_option0(args):
    tremo = TremoLoader(args.port)
    tremo.connect()
    tremo.set_baudrate(args.baud)
    return tremo.read_option0()


def tremo_write_option1(args):
    tremo = TremoLoader(args.port)
    tremo.connect()
    tremo.set_baudrate(args.baud)
    tremo.write_option1(args.option1l, args.option1h)


def tremo_read_option1(args):
    tremo = TremoLoader(args.port)
    tremo.connect()
    tremo.set_baudrate(args.baud)
    return tremo.read_option1()
...
if __name__ == '__main__':
    ...
    # write_option0
    parser_write_option0 = subparsers.add_parser(
        'write_option0',
        help='write the flash option0')
    parser_write_option0.add_argument('option0l', help='flash option0 low 32 bits', type=arg_int)
    parser_write_option0.add_argument('option0h', help='flash option0 low high bits', type=arg_int)

    # read_option0
    parser_read_option0 = subparsers.add_parser(
        'read_option0',
        help='read the flash option0')

    # write_option1
    parser_write_option1 = subparsers.add_parser(
        'write_option1',
        help='write the flash option1')
    parser_write_option1.add_argument('option1l', help='flash option1 low 32 bits', type=arg_int)
    parser_write_option1.add_argument('option1h', help='flash option1 low high bits', type=arg_int)

    # read_option1
    parser_read_option1 = subparsers.add_parser(
        'read_option1',
        help='read the flash option1')
    ...
    try:
        ...
        elif args.command == 'write_option0':
            tremo_write_option0(args)
            print('Write option0 successfully')
        elif args.command == 'read_option0':
            option0 = tremo_read_option0(args)
            print('Option0 value is: %08X %08X' % (option0[0], option0[1]))
        elif args.command == 'write_option1':
            tremo_write_option1(args)
            print('Write option1 successfully')
        elif args.command == 'read_option1':
            option1 = tremo_read_option1(args)
            print('Option1 value is: %08X %08X' % (option1[0], option1[1]))
